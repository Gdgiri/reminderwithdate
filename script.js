...
// Define variables for event input fields
...
function deleteEvent(eventId) {
  ...
  if (eventIndex !== -1) {
    ...
    // Save events to local storage after deletion
    localStorage.setItem("events", JSON.stringify(events));
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    if (
      ...
      listItem.innerHTML = `<strong>${event.title}</strong> - 
			...
			${eventDate.toLocaleDateString()} ${eventDate.toLocaleTimeString()}`; // Display time as well
